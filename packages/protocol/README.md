...
Since **0.8.0** a captured request may carry `poolWaitMs`: how long it waited for a connection from a pool before it could talk to the dependency at all. The instrumentation measures this per request already, and a consumer that only sees the total time cannot tell a slow database from one it never got to talk to. **Absent means the request asked no pool**, which is not a wait of zero — zero is a request that asked and was served at once — and the two have to stay apart, because a route that never queues must not read as one that queues for nothing.

Since **0.9.0** an error says **how the instrumentation came to see it**, and may carry the structural context the application attached to it. `Operation.kind` and `ProcessException.kind` each gain `framework` — the exception reached the framework's error path and was turned into a 5xx — and `explicit` — the application handed it over itself, from a `catch` that no hook can see. Both may carry a `context`: a flat object of at most **8** keys whose values are strings of at most **64** characters, already sanitised by the sender under the same rule as an error message, so anything that looked like a value arrives as `?` and nothing in it is per user. It is the context of the **first** occurrence of that signature in the window, because what travels here is counted per signature and never per occurrence. Absent is the normal case, and an instrumentation older than this sends neither the new kinds nor the field.
...
And since **0.9.0** a process exception may say what it already said. `ProcessException` may carry `total`: how many times its signature has happened in its instance since that instance started, a running total every delivery of the signature carries again. `count` keeps its meaning —what happened since the last delivery the sender heard land— so `total - count` is what the sender knows was applied. With it the cloud applies a delivery once: it keeps the largest total it has applied per instance and signature and counts only what a delivery adds beyond it, and never more than its `count`. So a batch sent again because its answer was lost, sent twice, or overtaken by a later one adds nothing, and one that never arrived is counted by the next. It relies on `instance.id` being generated once per process, which is what the contract already says. Without it, which is what an older sender sends, the cloud counts `count` as delivered, as it always did, and says so where that count is shown.

Fields are only ever **added**, and always optional. Each addition bumps the minor version, and the batch's `protocol` field is an enum of every published minor, so a cloud that speaks 0.2 still accepts an agent that speaks 0.1. Removing or renaming a field, or moving a histogram's bucket bounds, would be a major version on a new path. Bucket bounds are declared in the schema itself (`x-latency-boundaries-ms`, `x-calls-per-request-boundaries`), and so are the versions themselves and the ingestion path (`x-ingest-path`), so they are part of the generated contract rather than a constant someone has to keep in sync. So is the first minor that carries each kind of operation and of process exception (`x-since`, beside each `kind` enum), which is how a consumer tells a kind of error a sender cannot send from one it did not send. A validator in strict mode has to be told these annotations are keywords.
