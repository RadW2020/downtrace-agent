...
- **Benchmarks that contradict ours**: the agent claims less than 1 ms added at p99, under 3 percentage points of CPU and under 64 MiB. `packages/bench` is how we measure it. It runs **after** a merge and never as a gate before one, on a runner of its own in this repository — on the shared runners of a busy pipeline it measures the neighbours instead of the agent — and it can also be launched by hand. Its README says how to run it and what makes a run worth trusting. If you measure something else, that is a bug report we want.
